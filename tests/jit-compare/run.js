...
function run(use_jit) {
    return new Promise(resolve => {
        const emulator = new V86({
            ...
            disable_jit: use_jit ? 0 : 1, log_level: 0,
        });
        emulator.add_listener("emulator-loaded", function() {
            ...
            const execute = () => {
                for(let i = 0; i < 50 && !cpu.in_hlt[0]; i++) cpu.main_loop();
                const out = [];
                ...
                resolve({ regs: out, cache: cpu.wm.exports["jit_get_cache_size"](), compiled, interp });
            };
...
for(let i = 0; i < 32; i++)
    if(a.regs[i] !== b.regs[i]) {
        console.log(`MISMATCH xmm${i>>2}[${i&3}]: interp=${a.regs[i].toString(16)} jit=${b.regs[i].toString(16)}`);
        ok = false;
    }
